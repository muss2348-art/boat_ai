# boat_ai_app.py
# BOATRACE AI v10.5 ZIP版
# 紐抜け対策AI・頭期待値AI・履歴保持・厳選AI/予想モード
...
HEADERS = {"User-Agent": "Mozilla/5.0", "Accept-Language": "ja,en-US;q=0.9,en;q=0.8"}
    ...
    for i, line in enumerate(lines):
        if re.search(r"\d{4}\s*/\s*[AB][12]", line):
            for cand in lines[i + 1:i + 8]:
                if cand and not re.search(r"\d|kg|F\d|L\d|/", cand) and all(x not in cand for x in ["支部", "出身", "年齢", "全国", "当地", "モーター", "ボート"]):
                    return cand.replace(" ", "")
    for line in lines:
...
def parse_stats_from_racer_text(text: str) -> Dict[str, float]:
    ...
    return {
        ...
        "motor_2": get(6), "motor_3": get(7), "boat_2": get(8), "boat_3": get(9),
    }
...
def parse_racer_from_text(lane: int, text: str) -> Racer:
    ...
    return Racer(lane=lane, name=name, klass=klass, **stats, data_status=" / ".join(status))
...
def parse_beforeinfo(html: str, racers: List[Racer]) -> None:
    ...
    for m in re.finditer(r"([1-6])\s+(F)?\.?([0-9]{2})", joined):
        lane = int(m.group(1))
        if lane in by_lane:
...
def calculate_scores(racers: List[Racer]) -> List[Racer]:
    for r in racers:
        ...
        score += r.national_2 * 0.34 + r.national_3 * 0.10
        score += r.local_2 * 0.16 + r.motor_2 * 0.22 + r.boat_2 * 0.08
        if r.avg_st:
...
def calculate_head_power(racers: List[Racer]) -> List[Racer]:
    ...
    one_weak = bool(one and (one.score < top_score - 8 or one.avg_st >= 0.19 or (one.display_time and avg_display and one.display_time > avg_display + 0.08) or one.klass == "B1"))
    ...
    for r in racers:
        ...
        if r.avg_st and avg_st:
            if r.avg_st <= avg_st - 0.03: hp += 7
            elif r.avg_st <= avg_st - 0.015: hp += 4
            elif r.avg_st >= avg_st + 0.03: hp -= 5
        if r.display_time and avg_display:
            if r.display_time <= avg_display - 0.06: hp += 9
            elif r.display_time <= avg_display - 0.03: hp += 5
            elif r.display_time >= avg_display + 0.08: hp -= 6
        if r.tenji_st:
            if r.tenji_st <= 0.06: hp += 5
            elif r.tenji_st <= 0.10: hp += 3
            elif r.tenji_st >= 0.18: hp -= 3
        if r.tenji_f: hp -= 7
        if r.motor_2 >= 40: hp += 6
        elif r.motor_2 >= 35: hp += 3
        elif r.motor_2 and r.motor_2 <= 25: hp -= 4
        if r.local_2 >= 45: hp += 5
        elif r.local_2 >= 35: hp += 2
        elif r.local_2 and r.local_2 <= 15: hp -= 3
        if one_weak and r.lane in [2, 3, 4, 5]: hp += 8
        if r.lane in [4, 5, 6] and r.klass == "A1": hp += 8
        if r.lane in [4, 5, 6] and r.display_time and avg_display and r.display_time <= avg_display - 0.04: hp += 5
        if r.lane in [4, 5, 6] and r.local_2 >= 45: hp += 5
        if r.lane == 6 and not (r.klass == "A1" and r.score >= top_score - 5): hp -= 6
        r.head_power = round(max(0, min(100, hp)), 1)
    return sorted(racers, key=lambda x: x.head_power, reverse=True)


def calculate_string_power(racers: List[Racer]) -> List[Racer]:
    ranked = calculate_scores(racers)
    score_order = {r.lane: i + 1 for i, r in enumerate(ranked)}
    displays = [r.display_time for r in racers if r.display_time > 0]
    avg_display = sum(displays) / max(1, len(displays))
    for r in racers:
        sp = 30.0
        if r.display_time and avg_display:
            if r.display_time <= avg_display - 0.06: sp += 12
            elif r.display_time <= avg_display - 0.03: sp += 7
            elif r.display_time <= 7.18: sp += 4
        if r.avg_st <= 0.15: sp += 8
        elif r.avg_st <= 0.17: sp += 4
        if r.tenji_st:
            if r.tenji_st <= 0.08: sp += 7
            elif r.tenji_st <= 0.12: sp += 4
        if r.motor_2 >= 40: sp += 8
        elif r.motor_2 >= 35: sp += 5
        if r.boat_2 >= 40: sp += 4
        if r.local_2 >= 45: sp += 7
        elif r.local_2 >= 35: sp += 4
        if r.klass == "A1": sp += 8
        elif r.klass == "A2": sp += 4
        # 低指数艇の3着復活。最下位〜下位2艇は紐力を加点。
        if score_order.get(r.lane, 1) >= 5: sp += 10
        elif score_order.get(r.lane, 1) == 4: sp += 5
        if r.lane == 6 and sp < 50: sp -= 5
        r.string_power = round(max(0, min(100, sp)), 1)
    return sorted(racers, key=lambda x: x.string_power, reverse=True)
...
def tactic_analysis(racers: List[Racer]) -> Dict:
    ...
    one = by_lane.get(1)
    four = by_lane.get(4)
    in_score, upset_score, hole_score = 50.0, 35.0, 35.0
    comments = []
    if one:
        in_score += max(0, one.head_power - 65) * 0.6
        in_score += class_bonus(one.klass) * 0.5
        if one.avg_st <= 0.15:
            in_score += 7; comments.append("1号艇ST良好")
        elif one.avg_st >= 0.19:
            in_score -= 9; upset_score += 8; comments.append("1号艇ST遅め")
        if one.display_time and one.display_time <= 7.10:
            in_score += 7; comments.append("1号艇展示良好")
        elif one.display_time and one.display_time >= 7.22:
            in_score -= 9; upset_score += 8; comments.append("1号艇展示弱め")
    if four:
        if four.avg_st <= 0.14:
            upset_score += 6; hole_score += 5; comments.append("4カドST注意")
        if four.display_time and four.display_time <= 7.10:
            upset_score += 5; hole_score += 4; comments.append("4号艇展示良好")
        if four.head_power >= head_ranked[0].head_power - 5:
            upset_score += 7; hole_score += 5; comments.append("4号艇頭期待値上位")
    top_head = head_ranked[0]
    if top_head.lane != 1:
        upset_score += 10; comments.append(f"{top_head.lane}号艇の頭期待値上位")
    ...
    if in_score >= 78 and by_lane.get(1) and by_lane[1].head_power >= head_ranked[0].head_power - 5:
        ...
        style = "混戦"
    return {"展開": style, "イン逃げ度": in_score, "1飛び警戒": upset_score, "穴期待度": hole_score, "頭期待1位": f"{top_head.lane}号艇 {top_head.name}", "コメント": " / ".join(comments[:5]) if comments else "大きな偏りなし"}
...
def race_confidence(racers: List[Racer]) -> Dict:
    ...
    top, second, sixth = ranked[0], ranked[1], ranked[5]
    ...
    confidence = 38 + max(0, top.score - 88) * 0.35 + max(0, top_gap) * 1.0 + max(0, head_gap) * 0.9
    confidence += data_ok_count * 0.8 + display_ok_count * 0.7
    if tactic["展開"] == "イン逃げ濃厚": confidence += 5
    elif tactic["展開"] == "1飛び警戒": confidence -= 2
    elif tactic["展開"] == "混戦": confidence -= 6
    if head_top.lane == 1: confidence += 2
    if top.klass == "A1" or head_top.klass == "A1": confidence += 3
    if top.tenji_f: confidence -= 10
    if top_gap < 3: confidence -= 8
    if head_gap < 4: confidence -= 5
    if spread < 18: confidence -= 7
    if display_ok_count < 6: confidence -= 5
    if data_ok_count < 6: confidence -= 4
    confidence = round(max(0, min(96, confidence)), 1)
    if confidence >= 90 and head_gap >= 8 and spread >= 25 and data_ok_count >= 6 and display_ok_count >= 6:
        grade = "熱🔥"
    elif confidence >= 80 and head_gap >= 5:
        grade = "厚張り候補"
    elif confidence >= 70:
        grade = "厳選候補"
    elif confidence >= 62:
        ...
        grade = "見送り寄り"
    return {"confidence": confidence, "grade": grade, "top_lane": top.lane, "top_name": top.name, "top_score": top.score, "head_lane": head_top.lane, "head_name": head_top.name, "head_power": head_top.head_power, "top_gap": round(top_gap, 1), "head_gap": round(head_gap, 1), "spread": round(spread, 1), "data_ok": data_ok_count, "display_ok": display_ok_count, "tactic": tactic}


def make_ai_table(racers: List[Racer]) -> pd.DataFrame:
    calculate_scores(racers); calculate_head_power(racers); calculate_string_power(racers)
    ranked = sorted(racers, key=lambda x: x.score, reverse=True)
    return pd.DataFrame([{"順位": i, "艇": r.lane, "選手": r.name, "級別": r.klass, "AI指数": r.score, "頭期待値": r.head_power, "紐期待値": r.string_power, "展示": r.display_time, "展示ST": f"{'F' if r.tenji_f else ''}{r.tenji_st:.2f}" if r.tenji_st else 0, "平均ST": r.avg_st, "全国2連率": r.national_2, "当地2連率": r.local_2, "モーター2連率": r.motor_2, "ボート2連率": r.boat_2, "データ状態": r.data_status} for i, r in enumerate(ranked, start=1)])
...
def combo_score(combo: Tuple[int, int, int], racer_map: Dict[int, Racer], tactic: Dict, odds: Dict[str, float]) -> float:
    ...
    s = r1.head_power * 1.08 + r2.score * 0.48 + r3.score * 0.30
    # 紐抜け対策：2着/3着は紐期待値を別加点。3着を強めに見る。
    s += r2.string_power * 0.20
    s += r3.string_power * 0.34
    if a == 1: s += 4.0
    if tactic["展開"] == "イン逃げ濃厚":
        s += 6 if a == 1 else -2
        if b in [2, 3, 4]: s += 3
        if c in [2, 3, 4, 5, 6]: s += 2
        ...
        if a in [3, 4, 5]: s += 8
        if b in [1, 4, 5]: s += 3
    if r1.head_power >= 78: s += 8
    elif r1.head_power >= 70: s += 5
    elif r1.head_power >= 62: s += 2
    if r1.klass == "A1": s += 5
    elif r1.klass == "A2": s += 2
    if r3.string_power >= 60: s += 4
    elif r3.string_power >= 52: s += 2
    if a == 6 and r1.head_power < 78: s -= 8
    if odd > 0:
        if 7.0 <= odd <= 35.0: s += 8
        elif 35.0 < odd <= 80.0: s += 3
        elif odd < 5.0: s -= 5
        elif odd > 100: s -= 8
    return round(s, 2)
...
def balance_predictions(df: pd.DataFrame, racers: List[Racer], tactic: Dict, pick_count: int, preset: str) -> pd.DataFrame:
    ...
    df["三着"] = df["買い目"].astype(str).str.split("-").str[2].astype(int)
    head_ranked = calculate_head_power(racers)
    string_ranked = calculate_string_power(racers)
    head_priority = [r.lane for r in head_ranked if r.head_power >= 58]
    if 1 not in head_priority: head_priority.append(1)
    if preset == "的中率重視":
        total, max_head1, min_head_candidates, string_add = min(pick_count, 6), 3, 3, 1
    elif preset == "標準":
        total, max_head1, min_head_candidates, string_add = pick_count, 4, 4, 2
    else:
        total, max_head1, min_head_candidates, string_add = max(pick_count, 12), 5, 5, 3
    selected, selected_keys, head_counts = [], set(), {i: 0 for i in range(1, 7)}
    for head in head_priority[:min_head_candidates]:
        ...
        for _, row in cand.iterrows():
            ...
            if head == 1 and head_counts[1] >= max_head1: continue
            selected.append(row); selected_keys.add(key); head_counts[head] += 1
            break
    style = tactic["展開"]
    if style == "イン逃げ濃厚": quota = {1: max_head1, 2: 1, 3: 1, 4: 1, 5: 1}
    elif style == "1飛び警戒": quota = {1: 2, 2: 2, 3: 2, 4: 2, 5: 2}
    elif style == "穴期待": quota = {1: 2, 2: 1, 3: 2, 4: 2, 5: 2}
    else: quota = {1: 2, 2: 2, 3: 2, 4: 1, 5: 1}
    for head in head_priority:
        ...
        for _, row in cand.iterrows():
            ...
            if key in selected_keys: continue
            if head == 1 and head_counts[1] >= max_head1: continue
            if head_counts[head] >= limit: continue
            selected.append(row); selected_keys.add(key); head_counts[head] += 1
            if len(selected) >= total: break
    # 紐抜け対策：指数下位だけど紐期待値が高い艇を3着に1〜3点混ぜる
    score_rank = {r.lane: i + 1 for i, r in enumerate(calculate_scores(racers))}
    string_targets = [r.lane for r in string_ranked if score_rank.get(r.lane, 1) >= 5 and r.string_power >= 50]
    added_string = 0
    for target in string_targets:
        if added_string >= string_add or len(selected) >= total: break
        cand = df[df["三着"] == target].sort_values("評価", ascending=False)
        for _, row in cand.iterrows():
            key = row["買い目"]
            if key in selected_keys: continue
            selected.append(row); selected_keys.add(key); added_string += 1
            break
    for _, row in df.sort_values("評価", ascending=False).iterrows():
        if len(selected) >= total: break
        key, head = row["買い目"], int(row["頭"])
        if key in selected_keys: continue
        if head == 1 and head_counts[1] >= max_head1: continue
        selected.append(row); selected_keys.add(key); head_counts[head] += 1
    out = pd.DataFrame(selected).drop(columns=["頭", "三着"], errors="ignore").reset_index(drop=True)
    labels = []
    for i, row in out.iterrows():
        if i < 2: labels.append("熱🔥")
        elif i < 5: labels.append("本線")
        elif i < total - 1: labels.append("抑え")
        else: labels.append("穴")
    out["区分"] = labels
    return out
...
def generate_predictions(racers: List[Racer], odds: Dict[str, float], pick_count: int = 10, preset: str = "標準") -> pd.DataFrame:
    ...
    for combo in itertools.permutations([1, 2, 3, 4, 5, 6], 3):
        ...
        rows.append({"買い目": key, "評価": combo_score(combo, racer_map, tactic, odds), "オッズ": odd if odd > 0 else "未取得"})
    ...
    df["勝負度"] = conf
    df["厚張りAI"] = ""
    if conf >= 80: df.loc[df.index[:2], "厚張りAI"] = "候補"
    if conf >= 90: df.loc[df.index[:1], "厚張りAI"] = "強"
    return df


@st.cache_data(ttl=600, show_spinner=False)
def detect_today_venues(hd: str) -> List[str]:
    urls = [f"https://www.boatrace.jp/owpc/pc/race/index?hd={hd}", f"https://www.boatrace.jp/owpc/pc/race/index?hd={hd}&jcd="]
    found = set()
    for url in urls:
        try:
            html = fetch_html(url)
            ...
            for a in soup.find_all("a", href=True):
                ...
                if m:
                    ...
                    if jcd in JCD_MAP and ("racelist" in href or "race" in href or JCD_MAP[jcd] in txt): found.add(jcd)
            text = clean_text(soup.get_text(" "))
            for jcd, place in JCD_MAP.items():
                if place in text: found.add(jcd)
        except Exception:
            pass
    return sorted(found)
...
def analyze_single_race_cached(jcd: str, rno: int, hd: str, use_before: bool, use_odds: bool, pick_count: int, preset: str) -> Dict:
    ...
    odds = {}
    if use_odds:
        try: odds, _ = fetch_and_parse_odds3t(str(rno), jcd, hd)
    ...
    return {"場": place, "R": rno, "レース": f"{place}{rno}R", "勝負度": conf["confidence"], "判定": conf["grade"], "展開": tactic["展開"], "頭期待1位": tactic["頭期待1位"], "イン逃げ度": tactic["イン逃げ度"], "1飛び警戒": tactic["1飛び警戒"], "穴期待度": tactic["穴期待度"], "本命": f"{conf['top_lane']}号艇 {conf['top_name']}", "頭候補": f"{conf['head_lane']}号艇 {conf['head_name']}", "本命指数": conf["top_score"], "頭期待値": conf["head_power"], "上位差": conf["top_gap"], "頭期待差": conf["head_gap"], "全体差": conf["spread"], "展示取得": conf["display_ok"], "データ取得": conf["data_ok"], "買い目候補": top_picks, "URL": racelist_url, "開催名": meta.get("event_name", "")}


def build_note_text(event_name: str, place: str, rno: str, ai_df: pd.DataFrame, pred_df: pd.DataFrame, tactic: Dict) -> str:
    lines = []
    title = f"{place}{rno}R"
    if event_name: title += f"｜{event_name}"
    lines += [title, "", f"展開判定：{tactic['展開']}｜頭期待1位：{tactic['頭期待1位']}", f"イン逃げ度 {tactic['イン逃げ度']}%｜1飛び警戒 {tactic['1飛び警戒']}%", "", "【指数表・印】"]
    marks = ["◎", "○", "▲", "△", "☆", "消"]
    for i, row in ai_df.iterrows():
        mark = marks[i] if i < len(marks) else ""
        lines.append(f"{mark} {int(row['艇'])}号艇 {row['選手']}｜指数 {row['AI指数']}｜頭期待 {row['頭期待値']}｜紐期待 {row['紐期待値']}")
    lines += ["", "【買い目】"]
    for _, row in pred_df.iterrows():
        odds_txt = f"（{row['オッズ']}倍）" if isinstance(row["オッズ"], float) else "（オッズ未反映）"
        lines.append(f"{row['区分']} {row['買い目']} {odds_txt}")
    lines += ["", "※指数表・印とは連動していない場合もございます。", "※オッズ未取得の場合は指数・展示・成績・頭期待値・紐期待値を中心に評価しています。"]
    return "\n".join(lines)
...
def run_single_race_view(url: str, use_before: bool, use_odds: bool, pick_count: int, preset: str, debug: bool):
    ...
    odds, odds_meta = {}, {}
    if use_odds: odds, odds_meta = fetch_and_parse_odds3t(rno, jcd, hd)
    ai_df = make_ai_table(racers)
    pred_df = generate_predictions(racers, odds, pick_count, preset)
    conf = race_confidence(racers); tactic = conf["tactic"]
    st.subheader(f"{place} {rno}R")
    if meta.get("event_name"): st.write(meta["event_name"])
    ...
    st.text_area("コピー用", value=build_note_text(meta.get("event_name", ""), place, rno, ai_df, pred_df, tactic), height=340)
    if debug:
        st.markdown("### デバッグ")
        st.code(f"出走表: {racelist_url}\n直前情報: {before_url}")
        st.write("抽出方式:", meta.get("method")); st.write("オッズ件数:", len(odds)); st.write(odds_meta.get("odds_sample", {}))
...
def set_all_venues(value: bool): st.session_state["venue_selected"] = {jcd: value for jcd in JCD_MAP.keys()}
def set_detected_venues(detected: List[str]): st.session_state["venue_selected"] = {jcd: (jcd in detected) for jcd in JCD_MAP.keys()}
...
def venue_checkbox_selector(detected: List[str]) -> List[str]:
    ...
    with col_a:
        if st.button("自動検出だけ選択"):
            set_detected_venues(detected); st.rerun()
    with col_b:
        if st.button("全選択"):
            set_all_venues(True); st.rerun()
    with col_c:
        if st.button("全解除"):
            set_all_venues(False); st.rerun()
    cols = st.columns(4)
...
def compact_columns(df: pd.DataFrame) -> pd.DataFrame:
    cols = ["場", "R", "レース", "勝負度", "判定", "展開", "頭期待1位", "イン逃げ度", "1飛び警戒", "穴期待度", "本命", "頭候補", "本命指数", "頭期待値", "上位差", "頭期待差", "全体差", "展示取得", "データ取得", "買い目候補"]
    return df[[c for c in cols if c in df.columns]]

def detail_columns(df: pd.DataFrame) -> pd.DataFrame:
    cols = ["場", "R", "レース", "勝負度", "判定", "展開", "頭期待1位", "本命", "頭候補", "本命指数", "頭期待値", "上位差", "頭期待差", "全体差", "買い目候補", "開催名", "URL"]
    return df[[c for c in cols if c in df.columns]]
...
def show_saved_results(min_conf: int, max_rows: int):
    ...
    with tab1:
        st.markdown("### 全レース一覧"); st.dataframe(compact_columns(df_all), width="stretch", hide_index=True)
    with tab2:
        st.markdown("### 本日の厳選レース")
        if len(df_best) == 0: st.info("条件に合うレースはありませんでした。最低勝負度を下げてください。")
        else:
            ...
            lines += ["", "※指数・展示・成績・頭期待値・紐期待値を中心に自動評価しています。", "※指数表・印とは連動していない場合もございます。"]
            st.text_area("Note貼り付け用", value="\n".join(lines), height=300)
    with tab3:
        st.markdown("### 熱🔥・厚張り候補")
        if len(df_hot) == 0: st.info("熱🔥・厚張り候補はありませんでした。")
        else: st.dataframe(detail_columns(df_hot), width="stretch", hide_index=True)
    with tab4:
        st.markdown("### 開催場別")
        ...
        st.markdown(f"#### {place_choice} 全レース"); st.dataframe(compact_columns(df_place), width="stretch", hide_index=True)
        st.markdown(f"#### {place_choice} 厳選レース")
        if len(df_place_best) == 0: st.info("この開催場では条件に合う厳選レースがありません。")
...
def main():
    ...
    st.title("🚤 BOATRACE AI"); st.caption(APP_VERSION)
    if "last_results_df" not in st.session_state: st.session_state["last_results_df"] = None
    if "last_hd" not in st.session_state: st.session_state["last_hd"] = None
